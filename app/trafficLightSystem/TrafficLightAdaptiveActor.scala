...
/**
  ...
  */
class TrafficLightAdaptiveActor(carSpeed: Int = 5, routeCapacity: Int = 60) extends Actor {
  ...
  private var isUnderAdaptation: Boolean = false
  ...
  def handleNewCar(car: RealCar) = {
    if (!car.arrived()) {
      ...
      if (queues(car.entranceDirection)(car.nextTrafficLightDirection).size > totalQueueSize / 6) {
        isUnderAdaptation = true
        timings(car.entranceDirection)(car.nextTrafficLightDirection) =
          Math.ceil(timings(car.entranceDirection)(car.nextTrafficLightDirection) * 2).toInt
