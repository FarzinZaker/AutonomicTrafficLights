...
class TrafficLightAdaptiveWithAssuranceGrid {
  ...
  private val gridActors = Array.ofDim[TrafficLightAdaptiveWithAssuranceActor](rowsCount, columnsCount)


  val system = ActorSystem("TRAFFIC_LIGHT_ADAPTIVE_WITH_ASSURANCE_SYSTEM", com.typesafe.config.ConfigFactory.parseString(
    """ prio-dispatcher {
            type = "Dispatcher"
            mailbox-type = "%s"
  ...
  for {
    ...
    grid(i)(j) = system.actorOf(Props(new TrafficLightAdaptiveWithAssuranceActor()).withDispatcher("prio-dispatcher"), s"TRAFFIC_LIGHT_${i + 1}_${j + 1}")
  }
  ...
  def getStatus: JsObject = {
    ...
    for (i <- 1 to rowsCount) {
      nodes += JsObject(Seq(
        ...
        "loc" -> JsString(s"${i * 200 + 25.5} ${0 * 120}")
      ))
      nodes += JsObject(Seq(
        ...
        "color" -> JsString("white"),
        "textColor" -> JsString("#000000"),
        "loc" -> JsString(s"${i * 200 + 26} ${(columnsCount + 1) * 120}")
      ))
    }
