...
import play.api.libs.json.{JsNumber, JsString, JsArray, JsObject}
...
class TrafficLightGrid {
  ...
  private val gridActors = Array.ofDim[TrafficLightActor](rowsCount, columnsCount)
  ...
  for {
    ...
    grid(i)(j) = system.actorOf(Props(new TrafficLightActor()).withDispatcher("prio-dispatcher"), s"TRAFFIC_LIGHT_${i + 1}_${j + 1}")
  }
